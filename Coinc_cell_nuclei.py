...
root_path=r"/Users/Mathew/Documents/Edinburgh Code/Lengths_new_ONI.py/Test_data/"
pathlist=[]     
Pixel_size=103
scale=8
# Add paths below
...
Cell_filename="C1_cell.tiff"
Nucleus_filename="C1_nuclei.tiff"
...
def analyse_labelled_image(labelled_image,comparison):
    
    measure_image=measure.regionprops_table(labelled_image,intensity_image=comparison,properties=('area','perimeter','centroid','orientation','major_axis_length','minor_axis_length','mean_intensity','max_intensity'))
...
for path in pathlist:
    ...
    Cell=io.imread(path+Cell_filename).astype('int')
    Nucleus=io.imread(path+Nucleus_filename).astype('int')
    ...
    nucleus_overlap=analyse_labelled_image(SR,Nucleus)
    cell_overlap=analyse_labelled_image(SR,Cell)
    
    SR_stats['Cell_number']=cell_overlap['max_intensity']
    SR_stats['Nucleus_number']=nucleus_overlap['max_intensity']
    ...
    SR_stats.to_csv(path + 'Metrics_Coinc.csv', sep = '\t')
